               ...
               unsigned int& min)
{
...
#ifdef DEBUG
    if ((nv % (2*threads.x)) != 0)
        printf("Error en kernel_minimizar1: el último bloque no está lleno\n\n");
#endif // DEBUG
    ...
    kernel_minimizar1<<<grid_minimizar, threads, sizeof(unsigned int)*threads.x>>>(p_d, c_d, infinito, cdi);
...
#ifdef DEBUG
    if ((nvi % (2*threads.x)) != 0)
        printf("Error en kernel_minimizar2: el último bloque no está lleno\n\n");
#endif // DEBUG
                      ...
                      const unsigned int infinito)
{
    ...
    cudaMemcpy(c_d, c_h, mem_size, cudaMemcpyHostToDevice);
    ...
    cudaMalloc((void**) &p_d, mem_size);
    // copy host memory to device
    cudaMemcpy(p_d, p_h, mem_size, cudaMemcpyHostToDevice);
                     ...
                     const unsigned int* reference)
{
    ...
    //printf("%.6f", time);
    printf("Runtime for SSSP4 algorithm is: %.6f ms\n", time);
