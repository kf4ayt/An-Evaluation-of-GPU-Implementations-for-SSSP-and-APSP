                       ...
                       unsigned int*  chi, unsigned int* cho, unsigned int*  cdi, unsigned int* cdo)
{
    ...
    kernel1_SSSP6<<<grid,threads,threads.x*sizeof(unsigned int)>>>(v_d, a_d, w_d, p_d, f_d, c_d);
...
//////////////////////////////////
void testGraph_SSSP6(const unsigned int nv, const unsigned int mem_size_V,
                     ...
                     const unsigned int* reference)
{
    ...
    while(!ultima){
        i++;
        ultima= ejecutarIteracion_SSSP6( i, 
                          grid, threads, 
                          nv, na,
    ...
    printf("Runtime for SSSP6 algorithm is: %.6f ms\n", time);
