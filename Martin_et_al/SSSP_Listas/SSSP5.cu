                       ...
                       unsigned int*  chi, unsigned int* cho, unsigned int*  cdi, unsigned int* cdo)
{
    ...
    kernel1_SSSP5<<<grid,threads,threads.x*sizeof(unsigned int)>>>(v_d, a_d, w_d, p_d, f_d, c_d);
...
//////////////////////////////////
void testGraph_SSSP5(const unsigned int nv, const unsigned int mem_size_V,
                     const unsigned int na, const unsigned int mem_size_A,
                     const unsigned int infinito, const unsigned int* v_h,
                     const unsigned int* a_h, unsigned const int* w_h,
                     const unsigned int* reference)
{
    ...
    while(!ultima){
        i++;
        ultima= ejecutarIteracion_SSSP5( i, 
                          grid, threads, 
                          nv, na,
    ...
    printf("Runtime for SSSP5 algorithm is: %.6f ms\n", time);
