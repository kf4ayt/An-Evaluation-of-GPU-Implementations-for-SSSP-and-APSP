                       ...
                       bool* p_d, bool* f_d, unsigned int* c_d)
{
	...
	for(i=0; i<nv; i++){
		f_h[i]= false;  
                ...
                if(p_h[i] && (c_h[i]==minimo)){
			f_h[i]= true;
			p_h[i]= false;
		}//if
	}//for

    //Llevar de host a device las actualizaciones de F y P
    copiarH2D((void*)f_d, (void*)f_h, mem_size_F); 
    copiarH2D((void*)p_d, (void*)p_h, mem_size_F); 

    return (minimo==infinito);

}//ejecutarIteracion_SSSP1


void testGraph_SSSP1(const unsigned int nv, const unsigned int mem_size_V,
                     ...
                     const unsigned int* reference)
{
    ...
    while (!ultima) {
        i++;
        ultima = ejecutarIteracion_SSSP1(i, 
                                         grid, threads, 
                                         nv, na,
    ...
    printf("Runtime for SSSP1 algorithm is: %.6f ms\n", time);
